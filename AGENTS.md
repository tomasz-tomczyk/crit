...
6. **Diff hunk rendering** — code files show git diffs with dual gutters (old/new line numbers). Mounted code diffs always use row virtualization in `web/crit-diff-virtualizer.js` (unified + split), including story chapter bodies. Flat reviews also use **file-list virtualization** (`web/crit-file-list-virtualizer.js`): off-screen files are HeightIndex spacers; near-viewport files mount a real `.file-section` + row-virtualized body. File jumps use a pending stick target (device-pixel settle; cleared on user scroll intent) — see `docs/frontend-js.md`. The row virtualizer scrolls the nearest overflow parent (`#storyPane` in story mode).
