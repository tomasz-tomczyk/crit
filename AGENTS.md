...
6. **Diff hunk rendering** — code files show git diffs with dual gutters (old/new line numbers). Mounted code diffs always use row virtualization in `web/crit-diff-virtualizer.js` (unified + split); the Load Diff gate only defers fetching huge bodies.
