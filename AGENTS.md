...
6. **Diff hunk rendering** — code files show git diffs with dual gutters (old/new line numbers). Mounted code diffs always use row virtualization in `web/crit-diff-virtualizer.js` (unified + split), including story chapter bodies. On this spike branch (`spike/large-diff-file-list-virtualize`), `#filesContainer` also uses **file-list virtualization** (`web/crit-file-list-virtualizer.js`): off-screen files are HeightIndex spacers with estimated heights; near-viewport files mount a real `.file-section` + row-virtualized body, with scroll anchoring on height refine (Pierre CodeView-style). Compare with approach A+C on `spike/large-diff-virtualize` (deferred bodies keep empty open `<details>` + estimated `min-height` spacers). The row virtualizer scrolls the nearest overflow parent (`#storyPane` in story mode).
