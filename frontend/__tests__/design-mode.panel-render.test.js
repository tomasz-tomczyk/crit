// design-mode.panel-render.test.js — granular update path for the
// comments-side panel.
//
...
const { create } = require('../design-mode.panel-render.js');
...
// Render module reads window.crit.design.row for renderDesignPinRow. The
...
function setupDom() {
  ...
  const win = { crit: { designUtils: require('../design-route-utils.js'), design: {} } };
  global.document = doc;
  global.window = win;
...
function snapshotIds(panelBody) {
  ...
  for (const group of panelBody.childNodes) {
    ...
    if (!cards) continue;
    out.push({
      route: group.dataset.designRoute,
