// live-mode.comments-loader.js — fetches the canonical live-mode
...
// at boot. In live mode the very first `/api/session` returns
...
(function (root, factory) {
  ...
  if (typeof module === 'object' && module.exports) module.exports = api;
    ...
    root.crit.live.commentsLoader = api;
  }
  ...
  //
  // deps:
  //   state   — live state object (we read state.currentRound and write
    ...
    async function loadAllComments() {
      ...
      try {
        ...
        if (typeof console !== 'undefined' && console.warn) {
          console.warn('[live-mode] session refresh failed:', e);
        }
      }
      ...
      var results = await Promise.all(files.map(function (p) {
        return shared.fetchJSON('/api/file/comments?path=' + encodeURIComponent(p))
          ...
          .catch(function (e) {
            if (typeof console !== 'undefined' && console.warn) {
              console.warn('[live-mode] failed to load comments for', p, e);
            }
            return [];
          });
      }));
