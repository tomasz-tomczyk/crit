...
test.describe('design-mode pin click — P0 regression repro', () => {
  ...
  test('clicking a pin target opens the composer', async ({ page }) => {
    ...
    // events on Page for every frame; we filter to the design iframe by
    ...
    const selectionMsg = await page.evaluate(async () => {
      ...
      const log = () => (window as unknown as {
        __critDesignMessages?: { type: string; dom_anchor?: unknown }[]
    ...
    const allMessages = await page.evaluate(() => {
      return (window as unknown as { __critDesignMessages?: { type: string }[] })
        .__critDesignMessages || [];
    });

    const composerExists = await page.locator('.crit-design-composer').count();
    ...
    await expect(page.locator('.crit-design-composer')).toBeVisible({ timeout: 5_000 });
  });
});
