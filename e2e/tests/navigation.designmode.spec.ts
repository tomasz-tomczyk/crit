...
import { clearAllDesignPins, getIframe, setIframeRoute } from './designmode-helpers';
...
test.describe('navigation — link, pushState, redirects (Scenarios 10–13)', () => {
  ...
  test('pushState on /spa updates breadcrumb to /spa/section', async ({ page }) => {
    ...
    await expect(page.locator('#designRouteName')).toHaveText('/spa/section');
  });
  ...
  test.fixme('chrome breadcrumb reflects post-redirect path', async () => {
    // FIXME: chrome route announcer doesn't update #designRouteName when the
  ...
  test.fixme('cross-origin redirect surfaces an "open in real browser" affordance', async () => {
    // FIXME: shell.designmode.spec already has fixme'd coverage; the chrome
    // banner for cross-origin redirects is not yet wired (Phase B/E
    // incomplete). When [data-crit-design-cross-origin-banner] ships, replace
