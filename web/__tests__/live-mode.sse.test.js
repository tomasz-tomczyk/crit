...
const { create } = require('../live-mode.sse.js');
...
test('round transition surfaces a reply that landed between rounds (DOM-asserted)', async () => {
  ...
  //
  // Setup uses the real panel-render module + a stub renderLivePinRow
  ...
  const panelRender = require('../live-mode.panel-render.js');
  require('../live-route-utils.js');

  // Minimal DOM — same shape as live-mode.panel-render.test.js.
      ...
      live: {
        row: {
          renderLivePinRow(c) {
            ...
            wrap.dataset.liveRoute = (c.dom_anchor && c.dom_anchor.pathname) || '/';
