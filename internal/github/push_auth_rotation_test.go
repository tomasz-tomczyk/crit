...
func writeCritFile(t *testing.T, critRoot string, cj session.CritJSON) {
	t.Helper()
	out, err := json.MarshalIndent(cj, "", "  ")
	if err != nil {
		t.Fatalf("marshal: %v", err)
	}
	path := session.ReviewPathsFor(critRoot).Review
...
func readCritFile(t *testing.T, critRoot string) session.CritJSON {
	t.Helper()
	data, err := os.ReadFile(session.ReviewPathsFor(critRoot).Review)
	if err != nil {
...
func replyIDsByLocalID(cj session.CritJSON, path string) map[string]int64 {
