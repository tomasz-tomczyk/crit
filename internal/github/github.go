...
// GetCtx is the context-aware PR list fetch. The warm-prime path on daemon
...
// SeedForTest pre-populates the cache so picker tests skip gh.
func (c *PRListCache) SeedForTest(data []PRSummary) {
	c.mu.Lock()
	c.data = data
	c.fetched = time.Now()
	c.mu.Unlock()
...
func IsStackedPR(info *PRInfo, vcs vcs.VCS) bool {
	if info == nil || vcs == nil {
		return false
	}
...
// EnsureSHAFetched delegates to vcs.EnsureSHAFetched (kept for callers in github package).
func EnsureSHAFetched(vcsInst vcs.VCS, sha, repoRoot, forkURL string) error {
	return vcs.EnsureSHAFetched(vcsInst, sha, repoRoot, forkURL)
}

// EnsureSHAFetchedJJ delegates to vcs.EnsureSHAFetchedJJ.
func EnsureSHAFetchedJJ(vcsInst vcs.VCS, sha, repoRoot, forkURL string) error {
	return vcs.EnsureSHAFetchedJJ(vcsInst, sha, repoRoot, forkURL)
}

// EnsureSHAFetchedSapling delegates to vcs.EnsureSHAFetchedSapling.
func EnsureSHAFetchedSapling(vcsInst vcs.VCS, sha, repoRoot, forkURL string) error {
	return vcs.EnsureSHAFetchedSapling(vcsInst, sha, repoRoot, forkURL)
}
...
func isDuplicateGHComment(comments []session.Comment, ghID int64, author string, startLine, endLine int, body string) bool {
...
func isDuplicateGHReply(replies []session.Reply, ghID int64) bool {
...
// findCommentByGitHubID searches all files in a session.CritJSON for a comment with the given GitHubID.
// Returns the file path, comment index, and true if found.
func findCommentByGitHubID(cj *session.CritJSON, ghID int64) (string, int, bool) {
...
func appendNewGHReplies(comments []session.Comment, ci int, childReplies []ghComment, names userNameCache, pendingDeletes map[int64]bool) int {
	added := 0
	for _, r := range childReplies {
		...
		comments[ci].Replies = append(comments[ci].Replies, session.Reply{
			ID:                 session.RandomReplyID(),
			Body:               r.Body,
...
func updateDuplicateRoot(
	cj *session.CritJSON, cf *session.CritJSONFile, gc ghComment, replyMap map[int64][]ghComment,
...
func mergeRootComment(cj *session.CritJSON, gc ghComment, replyMap map[int64][]ghComment, now string, names userNameCache, scope inheritedScope, pendingDeletes map[int64]bool, threadResolved map[int64]bool) int {
	...
	if !ok {
		cf = session.CritJSONFile{Status: "modified", Comments: []session.Comment{}}
	}
...
func mergeOrphanReplies(cj *session.CritJSON, roots []ghComment, replyMap map[int64][]ghComment, names userNameCache, pendingDeletes map[int64]bool) int {
...
// mergeGHComments appends GitHub PR comments into an existing session.CritJSON.
// Only includes RIGHT-side comments (comments on the new version of the file).
// Handles threading: root comments become top-level Comments, replies become session.Reply entries.
...
func mergeGHCommentsWithNames(cj *session.CritJSON, ghComments []ghComment, names userNameCache, scope inheritedScope, threadResolved map[int64]bool) int {
...
// A reply needs pushing if its GitHubID is 0 (local-only) and its parent session.Comment has a GitHubID (on GitHub).
//
// rewrite handles image markdown in reply bodies — see bucketsToGHComments.
// nil falls back to the strip rewriter.
func collectNewRepliesForPush(cf session.CritJSONFile, rewrite bodyRewriter) []ghReplyForPush {
	if rewrite == nil {
		rewrite = session.StripBodyRewriter
	}
...
func collectEditedForPush(cj session.CritJSON) []ghEditForPush {
...
func updateCritJSONWithEditedBodies(critPath string, succeeded []ghEditForPush) error {
	...
	return config.AtomicWriteFile(critPath, append(out, '\n'), 0644)
}
...
func critJSONToGHComments(cj session.CritJSON) []map[string]any {
...
func updateCritJSONWithGitHubIDs(critPath string, commentIDs map[string]int64, replyIDs map[replyKey]int64) error {
	data, err := os.ReadFile(session.ReviewPathsFor(critPath).Review)
	if err != nil {
		return err
	}
	var cj session.CritJSON
	...
	return config.AtomicWriteFile(session.ReviewPathsFor(critPath).Review, append(out, '\n'), 0644)
}
