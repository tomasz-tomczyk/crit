...
func TestAddCommentToCritJSON_CreatesNewFile(t *testing.T) {
	...
	data, err := os.ReadFile(filepath.Join(outputDataRootIdentity(t, dir), "review.json"))
	if err != nil {
...
func TestAddCommentToCritJSON_AppendsToExisting(t *testing.T) {
	...
	data, _ := os.ReadFile(filepath.Join(outputDataRootIdentity(t, dir), "review.json"))
	var cj CritJSON
...
func TestAddCommentToCritJSON_OutputDir(t *testing.T) {
	...
	json.Unmarshal(data, &cj)
	if _, ok := cj.Files["main.go"]; !ok {
		t.Error("expected main.go comment under output data root")
	}
}
...
func TestAddCommentToCritJSON_RespectsBaseBranchConfig(t *testing.T) {
	...
	testutil.WriteFile(t, dir+"/base.go", "package main\n")
...
func TestAddReplyToCritJSON(t *testing.T) {
	...
	os.WriteFile(mustMkdirAll(filepath.Join(outputDataRootIdentity(t, dir), "review.json")), data, 0644)
...
func TestAddReplyToCritJSON_FallbackByCommentID(t *testing.T) {
	...
	os.WriteFile(mustMkdirAll(filepath.Join(outputDataRootIdentity(t, localDir), "review.json")), localData, 0644)
	...
	localData2, _ := os.ReadFile(filepath.Join(outputDataRootIdentity(t, localDir), "review.json"))
...
func TestUpdateCritJSONWithGitHubIDs(t *testing.T) {
	...
	data, _ := json.MarshalIndent(cj, "", "  ")
	os.WriteFile(mustMkdirAll(review.ReviewPathsFor(critPath).Review), data, 0644)
	...
	result, _ := os.ReadFile(review.ReviewPathsFor(critPath).Review)
	var got CritJSON
...
// readCritJSON is a test helper that reads and parses review.json under a data root.
...
func TestBulkAddCommentsToCritJSON_RedirectsRepliesToAltFile(t *testing.T) {
	...
	altData, err := os.ReadFile(review.ReviewPathsFor(altPath).Review)
	if err != nil {
		t.Fatal(err)
	}
	var alt CritJSON
...
func TestBulkAddCommentsToCritJSON_MultipleFiles(t *testing.T) {
	...
	os.Chdir(dir)
	defer os.Chdir(oldDir)
	testutil.WriteFile(t, filepath.Join(dir, "a.go"), "package a\n")
	testutil.WriteFile(t, filepath.Join(dir, "b.go"), "package b\n")
	entries := []BulkCommentEntry{
...
func TestUpdateCritJSONWithGitHubIDs_ReplyMapping(t *testing.T) {
	...
	data, _ := json.MarshalIndent(cj, "", "  ")
	os.WriteFile(mustMkdirAll(review.ReviewPathsFor(critPath).Review), append(data, '\n'), 0644)
...
func TestAddCommentToCritJSON_RoundTrip(t *testing.T) {
	...
	critPath := outputDataRootIdentity(t, dir)
...
func TestClearCritJSON(t *testing.T) {
	...
	if _, err := os.Stat(critPath); err != nil {
		t.Fatal("expected review identity to exist")
	}
	...
	if _, err := os.Stat(critPath); !os.IsNotExist(err) {
		t.Error("expected review identity to be deleted")
	}
}
...
func TestAddCommentToCritJSON_PopulatesAnchor(t *testing.T) {
	...
	testutil.WriteFile(t, filepath.Join(dir, "hello.go"), "package main\n\nfunc main() {\n\tfmt.Println(\"hello\")\n}\n")
...
func TestBulkAddCommentsToCritJSON_PopulatesAnchor(t *testing.T) {
	...
	testutil.WriteFile(t, filepath.Join(dir, "server.go"), "package main\n\nimport \"net/http\"\n\nfunc handler() {}\n")
