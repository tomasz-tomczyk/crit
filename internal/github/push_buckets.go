...
// scopedComment pairs a session.Comment with its file path and (when bucketed off the
...
func classifyComment(sc *scopedComment, currentHeadSHA string, inRangeMode bool) {
	c := sc.Comment
	if c.DiffScope == string(session.DiffScopeFullStack) {
	...
	if currentHeadSHA != "" && c.HeadSHA != currentHeadSHA {
		...
		sc.Detail = "was " + shortSHAString(c.HeadSHA)
		return
	}
}
...
func writeDryRunLine(sb *strings.Builder, sc scopedComment) {
	...
	stripped, _ := session.StripAttachmentReferences(sc.Comment.Body)
