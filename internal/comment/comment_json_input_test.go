...
func TestParseCommentFlagsFile(t *testing.T) {
	...
	got, err = parseCommentFlags([]string{"--json", "-f", "-"})
	if err != nil {
		t.Fatal(err)
	}
	if got.file != "-" {
...
func TestHelperProcess_RunCommentJSONFile(t *testing.T) {
	...
	if err := RunComment([]string{"--json", "--file", j, "--output", tmp, "--author", "tester"}); err != nil {
...
func TestHelperProcess_RunCommentJSONBad(t *testing.T) {
	...
	if err := RunComment([]string{"--json", "--file", bad, "--output", tmp}); err != nil {
		fmt.Fprintf(os.Stderr, "Error: %v\n", err)
		os.Exit(1)
	}
}
...
func TestHelperProcess_RunCommentJSONMissing(t *testing.T) {
	...
	tmp := t.TempDir()
	if err := RunComment([]string{"--json", "--file", filepath.Join(tmp, "does-not-exist.json"), "--output", tmp}); err != nil {
		fmt.Fprintf(os.Stderr, "Error: %v\n", err)
		os.Exit(1)
	}
}
