...
func readAnchorFromDisk(filePath string, startLine, endLine int) string {
	...
	return session.ExtractAnchor(string(data), startLine, endLine)
}

// appendReply adds a reply to an existing comment in the session.CritJSON struct in memory.
	...
	if err := appendReply(&cj, commentID, body, author, userID, resolve, filterPath); err != nil {
		...
		if strings.Contains(err.Error(), "not found") {
			if altPath, err2 := review.FindReviewFileByCommentID(commentID, critPath); err2 == nil {
				...
				return review.SaveCritJSON(altPath, altCJ)
			}
		}
		return err
	}
...
func processBulkFileOrLineEntry(cj *session.CritJSON, i int, e BulkCommentEntry, author, userID string, scope session.InheritedScope) error {
	filePath := e.File
	if filePath == "" {
		filePath = e.Path
	}
...
func processBulkLineComment(cj *session.CritJSON, i int, e BulkCommentEntry, cleaned, author, userID string, scope session.InheritedScope) error {
	startLine := e.Line
	endLine := e.EndLine
...
func bulkAddCommentsToCritJSONScoped(entries []BulkCommentEntry, globalAuthor, globalUserID string, outputDir string, scope session.InheritedScope) error {
	...
	if err := review.SaveCritJSON(targetPath, targetCJ); err != nil {
		return err
	}
	if redirected {
...
func resolveBulkTarget(entries []BulkCommentEntry, primaryPath string, primary session.CritJSON) (string, session.CritJSON, bool, error) {
	...
	for _, id := range replyIDs {
		if review.CJContainsCommentID(&primary, id) {
	...
	if len(inPrimary) > 0 {
		return "", session.CritJSON{}, false, fmt.Errorf(
	...
	for _, id := range missing {
		path, err := review.FindReviewFileByCommentID(id, primaryPath)
		if err != nil {
			return "", session.CritJSON{}, false, fmt.Errorf("reply target %s: %w", id, err)
		}
		if altPath == "" {
			altPath = path
			continue
	...
	altCJ, err := review.LoadCritJSON(altPath)
	if err != nil {
		return "", session.CritJSON{}, false, fmt.Errorf("load %s: %w", filepath.Base(altPath), err)
	}
...
// appendReviewCommentScoped adds a review-level comment to the session.CritJSON struct
...
func appendReviewCommentScoped(cj *session.CritJSON, body, author, userID string, scope session.InheritedScope) {
	...
	c := session.StampWithFocus(session.Comment{
		ID:        session.RandomReviewCommentID(),
		Body:      body,
		Author:    author,
		UserID:    userID,
		Scope:     "review",
...
// to the session.CritJSON struct in memory, stamping DiffScope/HeadSHA from scope.
