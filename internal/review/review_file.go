...
//
// This intentionally does NOT route through config.AtomicWriteFile. config.AtomicWriteFile
...
// all". Reusing config.AtomicWriteFile would require re-reading and re-writing the
...
func SaveCritJSON(critPath string, cj CritJSON) error {
	...
	return config.AtomicWriteFile(ReviewPathsFor(critPath).Review, append(data, '\n'), 0o644)
}
