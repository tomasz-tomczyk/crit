      ...
      "comments": [
        {
          ...
          "resolved": false,
          "replies": [
            { "id": "rp_c7d8e9", "body": "Fixed by extracting to helper", "author": "Grok" }
          ]
        }
      ]
    }
...
crit comment --author 'Grok' '<body>'

# File-level (whole file, no line numbers)
crit comment --author 'Grok' <path> '<body>'

# Line (single line or range)
crit comment --author 'Grok' <path>:<line> '<body>'
crit comment --author 'Grok' <path>:<start>-<end> '<body>'

# Reply to an existing comment
crit comment --reply-to <id> --author 'Grok' '<body>'
```

Hard rules:
- **Always pass `--author 'Grok'`** so comments are attributed correctly.
...
**For multi-paragraph bodies, prefer `--file`.** Write the JSON to a temp file, then:

```bash
crit comment --json --file /tmp/crit-bulk.json --author 'Grok'
```

`--file -` is an explicit "read stdin".
...
| `scope` | string | optional | `"review"` / `"file"` |
| `reply_to` | string | replies | Comment ID |
| `resolve` | bool | optional | Only when user explicitly asks |

## Plan-mode comments

Plan reviews store the review file in `~/.crit/plans/<slug>/`. **Always pass `--plan <slug>`**:

```bash
crit comment --plan my-plan-2026-03-23 --reply-to c_a1b2c3 --author 'Grok' 'Updated the plan'
```
...
```bash
crit pull [pr-number]
crit push [--dry-run] [--event <type>] [-m <msg>] [pr]
```

Requires `gh` CLI. `--event` can be `comment`, `approve`, or `request-changes`.
...
- Always relay the output to the user.
- `--qr` is terminal-only.
...
crit share --qr <file>        # Also print QR code (terminal only — Unicode blocks)
crit unpublish                # Remove shared review
```

- **No server needed** — reads files directly from disk. If a review file exists, its comments are included automatically.
...
- **Unpublish** uses the persisted `delete_token` stored in the review file.
</important>
