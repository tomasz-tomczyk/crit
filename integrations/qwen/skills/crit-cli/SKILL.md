  ...
  "review_comments": [
    {
      ...
      "resolved": false,
      "replies": [
        { "id": "rp_b4a5c6", "body": "Thanks, addressed the minor issues", "author": "Qwen" }
      ]
    }
  ],
  "files": {
      ...
      "comments": [
        {
          ...
          "resolved": false,
          "replies": [
            { "id": "rp_c7d8e9", "body": "Fixed by extracting to helper", "author": "Qwen" }
          ]
        }
      ]
    }
...
crit comment --author 'Qwen' '<body>'

# File-level (whole file, no line numbers)
crit comment --author 'Qwen' <path> '<body>'

# Line (single line or range)
crit comment --author 'Qwen' <path>:<line> '<body>'
crit comment --author 'Qwen' <path>:<start>-<end> '<body>'

# Reply to an existing comment
crit comment --reply-to <id> --author 'Qwen' '<body>'
```

Hard rules:
- **Always pass `--author 'Qwen'`** so comments are attributed correctly.
...
```bash
crit comment --json --file /tmp/crit-bulk.json --author 'Qwen'
```
...
```bash
crit comment --reply-to c_a1b2c3 --path src/auth.go --author 'Qwen' 'Fixed the null check'
```
...
```bash
crit comment --plan my-plan-2026-03-23 --reply-to c_a1b2c3 --author 'Qwen' 'Updated the plan'
```
