  ...
  "review_comments": [
    {
      ...
      "resolved": false,
      "replies": [
        { "id": "rp_b4a5c6", "body": "Thanks, addressed the minor issues", "author": "Cursor" }
      ]
    }
  ],
  "files": {
      ...
      "comments": [
        {
          ...
          "resolved": false,
          "replies": [
            { "id": "rp_c7d8e9", "body": "Fixed by extracting to helper", "author": "Cursor" }
          ]
        }
      ]
    }
...
```bash
crit comment --reply-to c_a1b2c3 --author 'Cursor' 'Fixed by extracting to helper'
...
```bash
crit comment --reply-to c_a1b2c3 --path src/auth.go --author 'Cursor' 'Fixed the null check'
```

In `--json` bulk mode, use the `file` field on the reply entry:

```bash
echo '[{"reply_to": "c_a1b2c3", "file": "src/auth.go", "body": "Fixed"}]' | crit comment --json --author 'Cursor'
```
...
crit comment --author 'Cursor' '<body>'

# File-level comment (about a file overall, no line numbers)
crit comment --author 'Cursor' <path> '<body>'
...
```bash
crit comment --author 'Cursor' 'Overall architecture looks solid'
crit comment --author 'Cursor' src/auth.go 'This file needs restructuring'
...
crit comment --reply-to c_a1b2c3 --author 'Cursor' 'Added null check on line 42'
crit comment --reply-to r_f1e2d3 --author 'Cursor' 'All issues addressed'
```

Rules:
- **Always use `--author 'Cursor'`** (or your agent name) so comments are attributed correctly
