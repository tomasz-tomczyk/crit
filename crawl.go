...
// crawlHTTPClient is used by crawlPreviewFromURL to fetch remote assets.
var crawlHTTPClient = &http.Client{Timeout: 15 * time.Second}

// cssEntry pairs a CSS file's relative path and content with the absolute URL
// it was fetched from, so url() references inside it can be resolved.
...
// crawlPreviewFromURL fetches an HTML page from a URL and all its local asset
// references, returning them as shareFile entries. This is the URL-based
// counterpart of crawlPreview (which reads from the filesystem).
func crawlPreviewFromURL(origin string) ([]shareFile, error) {
	htmlBytes, err := fetchAsset(origin)
	if err != nil {
		return nil, fmt.Errorf("cannot fetch %s: %w", origin, err)
	}
	...
	files = append(files, shareFile{Path: "index.html", Content: string(htmlBytes)})
	seen["index.html"] = true
	totalBytes += len(htmlBytes)

	cssFiles, err := collectHTMLAssetsFromURL(origin, htmlBytes, &files, seen, &totalBytes)
	if err != nil {
		return nil, err
	}

	if err := collectCSSAssetsFromURL(cssFiles, &files, seen, &totalBytes); err != nil {
		return nil, err
	}
...
// collectHTMLAssetsFromURL fetches all assets referenced in HTML and appends
// them to files. Returns CSS entries for further crawling.
func collectHTMLAssetsFromURL(origin string, htmlBytes []byte, files *[]shareFile, seen map[string]bool, totalBytes *int) ([]cssEntry, error) {
	var cssFiles []cssEntry
	for _, ref := range extractHTMLRefs(htmlBytes) {
		if isExternalURL(ref) {
			continue
		}
		relPath := cleanRelPathForURL(ref)
		if relPath == "" || seen[relPath] {
			continue
		}

		assetURL := resolveURL(origin, ref)
		data, fetchErr := fetchAsset(assetURL)
		if fetchErr != nil {
			continue
		}
		...
		*totalBytes += len(data)
		if *totalBytes > maxPreviewBytes {
			return nil, fmt.Errorf("preview snapshot exceeds %dMB limit", maxPreviewBytes/(1024*1024))
		}

		*files = append(*files, makeShareFile(relPath, data))
		seen[relPath] = true

		if strings.HasSuffix(strings.ToLower(relPath), ".css") {
			cssFiles = append(cssFiles, cssEntry{relPath: relPath, content: string(data), baseURL: assetURL})
		}
	}
	return cssFiles, nil
}

// collectCSSAssetsFromURL follows CSS files one level deep, fetching url()
// and @import references and appending them to files.
func collectCSSAssetsFromURL(cssFiles []cssEntry, files *[]shareFile, seen map[string]bool, totalBytes *int) error {
	for _, css := range cssFiles {
		cssDir := filepath.Dir(css.relPath)
		for _, cssRef := range extractCSSURLs(css.content) {
			if isExternalURL(cssRef) || strings.HasPrefix(cssRef, "data:") {
				continue
			}
			relPath := cleanRelPathForURL(filepath.Join(cssDir, cssRef))
			if relPath == "" || seen[relPath] {
				continue
			}
			cssAssetURL := resolveURL(css.baseURL, cssRef)
			data, fetchErr := fetchAsset(cssAssetURL)
			if fetchErr != nil {
				continue
			}
			*totalBytes += len(data)
...
// cleanRelPathForURL normalizes a URL reference path: strips query/fragment,
// percent-decodes, and rejects absolute or parent-traversal paths.
func cleanRelPathForURL(p string) string {
	if i := strings.IndexAny(p, "?#"); i >= 0 {
		p = p[:i]
	}
...
// fetchAsset fetches a URL and returns its body bytes, capped at maxPreviewBytes.
func fetchAsset(assetURL string) ([]byte, error) {
	resp, err := crawlHTTPClient.Get(assetURL)
	if err != nil {
		return nil, err
	}
	...
	return io.ReadAll(io.LimitReader(resp.Body, int64(maxPreviewBytes)))
}
