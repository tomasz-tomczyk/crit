// Sibling helpers for the live-mode Playwright project.
...
// (frontend/live-mode.js), not the speculative selectors in the original
...
/**
 * Bulk-delete every comment (live pins are comments). Same endpoint as
...
/**
 * Navigate to /live and wait for the iframe to be present and the agent
 * to post agent-ready (via __critLiveMessages, shipped by Phase B chrome).
 *
 * Note: a concurrent bug-fix round is addressing "browser doesn't auto-open
 * /live" — this helper navigates explicitly so it's unaffected.
 */
...
export async function exitPinMode(page: Page): Promise<void> {
  const navBtn = page.locator('#liveModeToggle button[data-mode="navigate"]');
  await navBtn.click();
...
export function getIframe(page: Page): FrameLocator {
  return page.frameLocator('#critLiveIframe');
}
...
export async function getMarkerCount(page: Page): Promise<number> {
  return getIframe(page).locator('.crit-live-marker').count();
}
...
export async function navigateInIframe(
  ...
  await expect(page.locator('#liveRouteName')).toHaveText(expectedRoute, { timeout: 10_000 });
}
...
/**
 * Seed a live pin directly via the API. Used to bypass the iframe-mediated
 ...
 */
export async function seedLivePin(
  ...
  if (!resp.ok()) {
    throw new Error(`seedLivePin failed: ${resp.status()} ${await resp.text()}`);
  }
 ...
 * at /live and you only want to switch to Pin mode and click a target —
 ...
 * clear `__critLiveMessages` BEFORE setIframeRoute() — see usages in
 * agent.livemode.spec.ts. This helper simply polls for any agent-ready.
 */
