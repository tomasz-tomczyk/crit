...
| ------------------ | ---------------------------- | ------------- | --------------------------------------------- |
| Queue              | Redis Streams, SQS, RabbitMQ | Redis Streams | Already in infra, no new services to manage   |
...
exposed to end users. No authentication required on the internal network.
...
The delivery worker runs as a separate long-lived process. It reads from the queue and
dispatches to channel handlers. Handlers are pluggable - email and webhook ship in the
MVP, others (Slack, SMS) can be added later without changing the worker core.

```mermaid
flowchart LR
    API[API Service] -->|enqueue| Queue[(Redis Stream)]
    Worker[Delivery Worker] -->|consume| Queue
...
Webhook requests time out after **5 seconds**. On a 4xx response (excluding 429),
